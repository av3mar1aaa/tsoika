...
Замеры этого приложения:

| | пик RSS |
|---|---|
| `next start` (работающий сайт) | **116 МБ**, после 120 запросов столько же |
| `next build` | **777 МБ** |

Сайту в работе хватает ~120 МБ, поэтому **1 ГБ RAM достаточно**. Ограничение
только на сборку — есть два способа её обойти, любой из них годится:

1. **Собирать на сервере, добавив swap** (см. шаг 1). Медленнее, но всё
   делается одной командой `deploy/deploy.sh`.
2. **Собирать на своей машине** и отправлять готовый `.next` —
   `deploy/deploy-from-local.sh`. На сервере тогда только
   `npm ci --omit=dev` и перезапуск. Для 1 ГБ это надёжнее.

**Ориентир по стоимости:** облачный сервер 1 vCPU / 1 ГБ / 15-20 ГБ NVMe —
порядка 400-500 ₽/мес (дешевле в питерских ЦОД и при годовой предоплате;
цены на сайте показаны со скидкой за год, помесячно ~11% дороже).
2 CPU / 2 ГБ / 40 ГБ — ~880 ₽/мес, нужны только если хочется собирать на
сервере без swap. Плюс домен `.ru` ~250 ₽/год и Object Storage ~0 ₽
(база 1.6 МБ + фото 28 МБ).

Чего брать **нельзя**:

- **Виртуальный хостинг** (у Timeweb от ~180 ₽) — там PHP/MySQL и нет
  постоянного Node-процесса. Next.js не запустится.
- **«App Platform»** и подобные PaaS — эфемерная файловая система,
  SQLite-файл сотрётся при первом же деплое.

Нужен именно **VDS/облачный сервер с root-доступом и постоянным диском**.
...
Swap. На 1 ГБ он обязателен, если собираете на сервере; на 2 ГБ — желателен
как страховка. Если собираете локально (`deploy-from-local.sh`), достаточно
1 ГБ swap «на всякий случай».

```bash
fallocate -l 2G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile
echo '/swapfile none swap sw 0 0' >> /etc/fstab
# на 1 ГБ RAM имеет смысл реже вытеснять в swap
...
Собираем на сервере (нужен swap) — либо пропускаем этот шаг, если будете
собирать локально через `deploy-from-local.sh`:
...
Сборка на сервере (нужно ≥2 ГБ RAM или swap):

```bash
ssh tsoika@СЕРВЕР '/srv/tsoika/deploy/deploy.sh'
```

Сборка на своей машине (для 1 ГБ RAM) — запускать локально из корня проекта:

```bash
SERVER=tsoika@СЕРВЕР ./deploy/deploy-from-local.sh
```

Обоим нужен `sudo systemctl restart tsoika` без пароля:
